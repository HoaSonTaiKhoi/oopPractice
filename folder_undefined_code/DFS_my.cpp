...
void step(int v, int e, map<char, int> v_index, map<int, char> a_index, vector<vector<int>> a, char t1, char t2)
{
    stack<char> open;
    ...
    while(!open.empty())
    {
        ...
        for(int i=0; i<v; i++)
        {
            ...
            a[v_index[p]][i]=0;
              ...
              if(check_stack(open, a_index[i]))open.push(a_index[i]);
